...
+ [修复] 严格 OpenAI 图片接口对 `gpt-image-*` 模型不再发送不兼容的 `response_format` 字段，文生图与图生图请求保持一致。
+ [优化] Agent 生成流程在提示词仅由已引用节点组成时复用原节点，避免创建冗余包装文本节点；含正文的提示词保留原有流程。
+ [优化] 合并官方 `v0.16.0` 的画布交互、图片生成和 API 兼容性更新，并保留本地任务持久化、Agent 隔离和性能优化。
+ [优化] 画布文本、图片、视频、音频首次生成统一保存稳定的 taskId、实际来源节点、结果键和状态，并在本地写入确认后记录 persistedAt；重试继承原任务和结果键，取消、刷新恢复及旧响应晚到不会重复或覆盖新结果。
+ [优化] 生成结果槽位记录幂等 resultKey，失败任务保留错误与输入上下文，便于安全重试。
+ [调整] 生成配置节点为文本、图片、视频、音频分别保存独立生成数量，数量范围统一为 1–15；普通节点仍固定生成 1 个，模式切换不会互相继承数量。
+ [修复] 生图工作台与画布左侧提示词库的长文本区域支持鼠标滚轮在框内滚动，提示词卡片和列表分别滚动且不再带动外层页面或画布。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [新增] 第三阶段图片任务接入本地 Canvas Agent 的 OpenAI 兼容图片适配器；未配置密钥时保留模拟结果。
+ [新增] 第四阶段支持单一 ComfyUI 工作流与本地视频模拟任务；未安装 ComfyUI 或未配置真实视频服务时均可明确降级，画布仍可独立编辑。
+ [修复] OpenAI 兼容图片任务会使用网页当前渠道配置并经本地 Agent 转发；网页与 Agent 都没有密钥时才返回明确标记的模拟预览。
+ [修复] 图生图上传会按参考图真实文件头校正 MIME 类型与扩展名，避免 JPEG 被错误声明为 PNG 后遭接口拒绝。
+ [新增] 画布图片配置节点支持网页 API、通用生图 Skill 和 AIGC CLI Skill；Skill 结果会保存到本机并可回填画布，Agent 不可用时按开关回退网页 API。
+ [新增] Agent 新增受控的本机生成图片回填工具，支持把 Skill 输出连接回当前画布流程，同时保留画布独立编辑能力。
+ [修复] Windows 下 AIGC CLI Skill 自动跳过失效的 Python 启动器条目，并兼容 GB18030 错误输出，避免任务无故回退或显示乱码。
