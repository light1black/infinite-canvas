...
export function applyNodeConfigPatch(node: CanvasNodeData, patch: Partial<CanvasNodeData["metadata"]>) {
    ...
    if (node.type === CanvasNodeType.Config && next.metadata?.generationMode === "image" && next.metadata?.imageExecutor && next.metadata.imageExecutor !== "api" && node.height < NODE_DEFAULT_SIZE[CanvasNodeType.Config].height) {
        const height = NODE_DEFAULT_SIZE[CanvasNodeType.Config].height;
        return { ...next, height, position: { x: node.position.x, y: node.position.y + node.height / 2 - height / 2 } };
    }
