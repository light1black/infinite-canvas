...
export function CanvasTextSettingsPopover({ config, onConfigChange, showCount = false, buttonClassName, placement = "topLeft" }: CanvasTextSettingsPopoverProps) {
    ...
    const panel = open && buttonRect ? <TextSettingsPortal buttonRect={buttonRect} panelRef={panelRef} placement={placement} theme={theme} config={config} onConfigChange={onConfigChange} showCount={showCount} /> : null;

    return (
        <>
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={buttonClassName || "!h-8 !max-w-[170px] !justify-start !rounded-full !px-2.5"} style={{ background: theme.node.fill, color: theme.node.text }} icon={<Settings2 className="size-3.5" />} onClick={() => setOpen((current) => !current)}>
                    <span className="truncate">{t("canvas.controls.reasoning")} · {reasoningEffortLabel(config.reasoningEffort)}{showCount ? ` · ${t("settingsPanels.text.results", { count })}` : ""}</span>
                </Button>
            </span>
            {panel}
        </>
...
function TextSettingsPortal({ buttonRect, panelRef, placement, theme, config, onConfigChange, showCount }: {
    buttonRect: DOMRect;
    ...
    return createPortal(
        <div ref={panelRef} style={style} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
            <TextSettingsPanel config={config} onConfigChange={onConfigChange} theme={theme} showCount={showCount} />
        </div>,
        document.body,
    );
}
