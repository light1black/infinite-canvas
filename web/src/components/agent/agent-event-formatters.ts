...
function toolInputRows(name: string, input: unknown) {
    ...
    if (name === "canvas_create_local_image_nodes") return [detailRow(tr("imageCount"), Array.isArray(objectField(input, "paths")) ? (objectField(input, "paths") as unknown[]).length : 0)].flatMap((row) => (row ? [row] : []));
    return [];
}
...
export function isCanvasWriteTool(name: string) {
    return name === "canvas_apply_ops" || name === "canvas_create_attachment_nodes" || name === "canvas_create_local_image_nodes";
}
